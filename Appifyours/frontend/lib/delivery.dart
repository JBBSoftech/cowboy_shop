...
//   3. Copy this file → lib/delivery.dart
//
// pubspec.yaml needs:
//   http: ^1.0.0
...
// Import MainScreen for navigation
  ...
  Widget _orderSummary() => Column(children: [
    ..._items.map((i) => Padding(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Text('₹${((i.effectivePrice as num) * (i.quantity as num)).toStringAsFixed(2)}',
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        _card('📦 Order Details', Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _row('Order ID', result.orderId),
            if (result.shipmentId.isNotEmpty) _row('Shipment ID', result.shipmentId),
            _row('Payment Method', paymentMethod == 'cod' ? 'Cash on Delivery' : 'Paid Online'),
            _row('Total Amount', '₹${grandTotal.toStringAsFixed(2)}'),
          ],
        )),
        const SizedBox(height: 16),
        _card('🚚 Delivery Info', Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _row('Courier', courier.courierName),
            _row('Est. Delivery', '${courier.estimatedDays} days'),
        ...
        _card('📍 Delivery Address', Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text('📱 ${address.phone}'),
            if (address.email.isNotEmpty) Text('✉️ ${address.email}'),
          ],
        )),
        const SizedBox(height: 30),
        SizedBox(
          ...
          child: OutlinedButton.icon(
            onPressed: () async {
              if (result.shipmentId.isNotEmpty) {
                final tracking = await service.trackOrder(result.shipmentId);
                if (context.mounted) {
                  showDialog(
                    context: context,
                    builder: (_) => AlertDialog(
                      ...
                      content: Column(
                        ...
                        children: [
                          ...
                          if (tracking.currentLocation.isNotEmpty) Text('Location: ${tracking.currentLocation}'),
                          if (tracking.estimatedDate.isNotEmpty) Text('Est. Date: ${tracking.estimatedDate}'),
                        ],
                      ),
                      actions: [
                        TextButton(
